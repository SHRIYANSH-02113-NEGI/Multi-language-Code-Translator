...
def convert_java_to_csharp(java_code):
    ...
    result = []
    indentation = 0  # track indentation level
    while True:
        tok = lexer.token()
        if not tok:
            break
        ...
        if tok.type == 'NEWLINE':
            result.append('\n' + '    ' * indentation)  # Add indentation with each newline
            continue

        if tok.type == 'LBRACE':  # Open block, increase indentation
            result.append(tok.value)
            indentation += 1
        elif tok.type == 'RBRACE':  # Close block, decrease indentation
            indentation -= 1
